<?php
		 ...
		 {
		 	global $inflationMultiplier;
		 	global $horsePrice;
		 	global $cowPrice;
		 	global $woolPrice;
		 	global $wheatPrice;
		 	global $wagesPrice;
				 $inflationMultiplier = $r;
				$horsePrice = $hp;
				$cowPrice = $cp;
				$woolPrice = $wop;
				$wheatPrice = $whp;
		...
		function set_inflation_rate($userInputYear) {
		...
		function convert_old_money_to_new($year,$pounds,$shilling,$pence){
			set_inflation_rate($year);
			global $inflationMultiplier;
			$modernCurrency = $pounds + ( $shilling / 20) + ( $pence / 240);
			return $modernCurrency * $inflationMultiplier;

		}

		function calculate_buying_power($userInputMoney,$userInputYear){
			set_inflation_rate($userInputYear);
			global $inflationMultiplier;
			global $horsePrice;
			global $cowPrice;
			global $woolPrice;
			...
			global $wagesPrice;
			global $money;
			$money = $userInputMoney / $inflationMultiplier;

			return array(
				"horses" => floor($money/$horsePrice),
				"cows" => 	floor($money/$cowPrice),
				"wool" => floor($money/$woolPrice),
				"wheat" => floor($money/$wheatPrice),
				"wages" => floor($money/$wagesPrice)
